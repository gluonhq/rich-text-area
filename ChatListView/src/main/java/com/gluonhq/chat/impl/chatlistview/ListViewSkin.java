...
public class ListViewSkin<T> extends VirtualContainerBase<ListView<T>, ListCell<T>> {
    ...
    private static final String EMPTY_LIST_TEXT = "ListView.noContent";
    ...
    private final EventHandler<KeyEvent> keyEventListener = e -> {
        if (e.getCode() != null) {
            switch(e.getCode()) {
                case PAGE_UP:
                case UP: verticalUnitDecrement(); break;
                case SPACE:
                case PAGE_DOWN:
                case DOWN: verticalUnitIncrement(); break;
                case HOME: verticalHome(); break;
                case END: verticalEnd(); break;
                default:
            }
            e.consume();
        }
    ...
    /**
     * This method is called when new data is added at a given index of the list
     * of items.
     *
     * @param index
     * @param size 
    ...
    protected void processInputData(int index, int size) {
        flow.processInputData(index, size);
    }
...
//        boolean isSelected = sm.isSelected(lastVisibleCellIndex) || fm.isFocused(lastVisibleCellIndex) || lastVisibleCellIndex == anchor;
    ...
    void verticalUnitIncrement() {
        flow.getVbar().increment();
    }
    
    void verticalUnitDecrement() {
        flow.getVbar().decrement();
    }
    
    void verticalHome() {
        flow.getVbar().setValue(flow.getVbar().getMin());
    }
    
    void verticalEnd() {
        flow.getVbar().setValue(flow.getVbar().getMax());
    }
