...
import com.gluonhq.chat.impl.chatlistview.util.Utils;
...
public class VirtualFlow<T extends IndexedCell> extends Region {
    ...
    private static final Logger LOG = Logger.getLogger(VirtualFlow.class.getName());
    ...
    private final ArrayLinkedList<Double> lengths = new ArrayLinkedList<>();
...
//        vbar.addChangedListener(ScrollBar.VALUE, listenerY);
        ...
        */
        ...
        totalLength = getTotalLength(false);
    }
        ...
        if (vertical == null) {
            vertical = new BooleanPropertyBase(true) {
                @Override protected void invalidated() {
                    ...
                    setNeedsLayout(true);
                    requestLayout();
                    totalLength = getTotalLength(true);
                }
    ...
    private DoubleProperty position = new SimpleDoubleProperty(this, "position") {
        @Override public void setValue(Number v) {
            super.setValue(Utils.clamp(0, get(), 1));
        }
...
//    }
    ...
    public double scrollPixels(final double delta) {
        ...
        if (cells.size() > 0) {
            ...
            if (layoutY > 0 && getCellIndex(firstCell) == 0 && getPosition() > 0) {
                // this shouldn't happen, but if it does, we move the cells back to the top
                layoutY = 0;
            }
            for (int i = 0; i < cells.size(); i++) {
                T cell = cells.get(i);
        ...
        // notify
        return (pos - lastPosition) * totalLength;
    }
     ...
     * any reason.
     * @param index the index
     * @param skipVisible if true, it doesn't try to retrieve the cell from the
     *                    visible cells
     * @return the cell
     */
    private T getCell(int index, boolean skipVisible) {
        // If there are cells, then we will attempt to get an existing cell
        if (! cells.isEmpty() && ! skipVisible) {
    ...
    private void setMaxPrefBreadth(double value) {
    ...
    private void setViewportBreadth(double value) {
        this.viewportBreadth = value;
    }
    private double getViewportBreadth() {
    ...
    double getCellLength(T cell) {
        ...
        return isVertical() ?
                cell.prefHeight(cell.getLayoutBounds().getWidth())
                : cell.prefWidth(cell.getLayoutBounds().getHeight());
    }
    ...
    private void positionCell(T cell, double position) {
        if (isVertical()) {
            cell.setLayoutX(0);
            cell.setLayoutY(snapSizeY(Utils.truncate(position)));
        } else {
            cell.setLayoutX(snapSizeX(Utils.truncate(position)));
            cell.setLayoutY(0);
        }
    }
        ...
        while (offset < viewportLength) {
            if (index >= cellCount) {
                ...
                if (index > maxCellCount) {
                    LOG.log(Level.FINE, "index exceeds maxCellCount. Check size calculations for " + startCell.getClass());
    ...
    private double getOffsetFromEmptyCells() {
        if (! stackFromBottom) {
            return 0;
        }
        
        double offset = getHeight() - (hbar.isVisible() ? hbar.getHeight() : 0);
    ...
    private void updateScrollBarsAndCells(boolean recreate) {
        ...
        if (!cells.isEmpty()) {
            final double currOffset = -computeViewportOffset(getPosition()) + getOffsetFromEmptyCells();
        ...
        if (recreate && (lengthBar.isVisible() || Properties.IS_TOUCH_SUPPORTED)) {
            ...
            if (numCellsVisibleOnScreen == 0 && cellCount == 1) {
                ...
                lengthBar.setVisibleAmount(sumCellLength / totalLength);
            }
        }
...
//            }
     ...
     * That is, if the return value of this function were used as a translation
    ...
    private void adjustPositionToIndex(int index) {
        ...
        if (cellCount <= 0) {
            ...
            setPosition(getRelativeLengthAtIndex(index));
        }
    }
    ...
    private void adjustByPixelAmount(double numPixels) {
        ...
        // by numPixels one cell at a time. For each cell, we discover how many
        ...
        int cellIndex = getCellIndexAtLength(position * totalLength);
        ...
        double cellPercent = cellSize / totalLength;
        ...
        double n = forward ?
              numPixels + computeViewportOffset(position) - start
            : -numPixels + end - computeViewportOffset(position);
        ...
        // value of Position.
        double p = getRelativeLengthAtIndex(cellIndex);
        ...
        if (n > remaining) {
            ...
            double newPosition = p + rate * n;
            if (newPosition < position) {
                // This shouldn't happen.
                // But if it does, instead of going backwards, against the scroll
                // direction, and causing a race condition, increase the position
                // by 5%
                newPosition = 1.05 * position;
            }
            setPosition(newPosition);
        } else {
            double rate = cellPercent / Math.abs(end - start);
            double newPosition = p + cellPercent - rate * n;
            if (newPosition > position) {
                // This shouldn't happen.
                // But if it does, instead of going forward, against the scroll
                // direction, and causing a race condition, decrease the position
                // by 5%
                newPosition = 0.95 * position;
            }
            setPosition(newPosition);
        }
    }
    ...
    private int computeCurrentIndex() {
        return getCellIndexAtPosition(getPosition());
    }
    ...
    private double computeOffsetForCell(int itemIndex) {
        int index = (int) Utils.clamp(0, itemIndex, getCellCount());
        return - getRelativeLengthAtIndex(index) * getViewportLength();
    }

    /***************************************************************************
     *                                                                         *
     * Utility methods for a VirtualFlow with cells of different length        *
     *                                                                         *
     **************************************************************************/

    /**
     * Whenever the control adds new items, update the lengths array with empty
     * cells at the beginning or at the end.
     *
     * The length values will be determined later on by {@link #getTotalLength(boolean)}.
     *
     * @param index
     * @param size
     */
    void processInputData(int index, int size) {
        for (int i = 0; i < size; i++) {
            if (index == 0) {
                lengths.addFirst(null);
            } else {
                lengths.addLast(null);
            }
        }
    }

    /**
     * Total length of cells, summing up its real length. It takes into account 
     * all the list of items.
     *
     * @param reset When true, regenerates the whole list
     * @return
     */
    private double getTotalLength(boolean reset) {
        // update length of new added cells
        IntStream.range(0, getCellCount())
                .filter(i -> reset || lengths.get(i) == null)
                .forEach(i -> {
                    accumCell = null;
                    pile.clear();
                    T cell = getCell(i, true);
                    Double h = getCellLength(cell);
                    releaseCell(cell);
                    if (lengths.size() > i) {
                        lengths.set(i, h);
                    } else {
    ...
    /**
     * Returns the accumulated length from the beginning of the cells to the start
     * of the cell at the given index.
     * 
     * @param index
     * @return 
     */
    private double getAcumLengthAtIndex(int index) {
        return lengths.stream()
                .limit(index)
                .reduce(0d, Double::sum);
    }
    ...
    /**
     * Returns the relative accumulated length from the beginning of the cells to 
     * the start of the cell at the given index. Between 0 and 1
     * 
     * @param index
     * @return 
     */
    private double getRelativeLengthAtIndex(int index) {
        return getAcumLengthAtIndex(index) / totalLength;
    }
    
    /**
     * Returns the index of the cell for which a given length lies between the 
     * accumulated length from the beginning of this cell and the beginning of the 
     * next one.
     * 
     * @param accumLength
     * @return 
     */
    private int getCellIndexAtLength(double accumLength) {
        double pos = 0;
        for (int i = 0; i < getCellCount(); i++) {
            pos += lengths.get(i);
            if (pos > accumLength) {
    ...
    /**
     * Given the cell for which a given length lies between the accumulated length
     * from the beginning of this cell and the beginning of the next one, it returns
     * the relative difference between the accumulated length and the beginning of
     * the cell.
     * 
     * @param accumLength
     * @return 
     */
    private double getCellOffsetAtLength(double accumLength) {
        double pos = 0;
        for (int i = 0; i < getCellCount(); i++) {
            double h = lengths.get(i);
            if (pos + h > accumLength) {
                return accumLength - pos;
            }
            pos += h;
        }
        return 0d;
    ...
    /**
     * Returns the index of the cell for which a relative length lies between the 
     * accumulated length from the beginning of this cell and the beginning of the 
     * next one.
     * 
     ...
     * @return 
     */
    private int getCellIndexAtPosition(double relativePosition) {
        double p = Utils.clamp(0, relativePosition, 1);
        return getCellIndexAtLength(p * totalLength);
    }
    ...
    static class ArrayLinkedList<T> extends AbstractList<T> {
        ...
        public T set(int index, T cell) {
            if (index > (lastIndex - firstIndex) || index < 0) {
                return null;
            }

            return array.set(firstIndex + index, cell);
        }
